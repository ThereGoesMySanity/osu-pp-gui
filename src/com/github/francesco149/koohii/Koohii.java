...
private static
double d_spacing_weight(int type, double distance, Controller c)
{
    switch (type)
    {
    case DIFF_AIM:
    ...
    case DIFF_SPEED:
        if (distance > SINGLE_SPACING) {
            return c.getVariable("singletap spacing weight", 2.5);
        }

        else if (distance > STREAM_SPACING)
        {
            return 1.6 + c.getVariable("stream spacing weight", 0.9) * (distance - STREAM_SPACING) /
                (SINGLE_SPACING - STREAM_SPACING);
        }

        else if (distance > ALMOST_DIAMETER)
        {
            return 1.2 + c.getVariable("spacing bonus 1", 0.4) * (distance - ALMOST_DIAMETER) /
                (STREAM_SPACING - ALMOST_DIAMETER);
        }

        else if (distance > ALMOST_DIAMETER / 2.0)
        {
            return 0.95 + c.getVariable("spacing bonus 2", 0.25) *
...
void d_strain(int type, HitObject obj, HitObject prev,
    double speed_mul, Controller c)
{
    double value = 0.0;
    double time_elapsed = (obj.time - prev.time) / speed_mul;
    double decay =
        Math.pow(c.getVariable((type==1?"aim":"speed")+" decay", DECAY_BASE[type]), time_elapsed / 1000.0);
    ...
    if ((obj.type & (OBJ_SLIDER | OBJ_CIRCLE)) != 0)
    {
        ...
        value = d_spacing_weight(type, distance, c);
        value *= c.getVariable((type==1?"aim":"speed")+" star weight", WEIGHT_SCALING[type]);
    }
...
public static class DiffCalc
{
    ...
    */
    ...
    private double calc_individual(int type, Controller c)
    {
        strains.clear();
        ...
        for (int i = 0; i < beatmap.objects.size(); ++i)
        {
            ...
            if (prev != null) {
                d_strain(type, obj, prev, speed_mul, c);
            }
            ...
            while (obj.time > interval_end)
            {
                ...
                if (prev != null)
                {
                    ...
                    strain */
                    double decay = Math.pow(c.getVariable((type==1?"aim":"speed")+" decay", DECAY_BASE[type]),
        ...
        for (Double strain : strains)
        {
            ...
            weight *= c.getVariable("decay weight", DECAY_WEIGHT);
        }
    ...
    * @return self
    */
    public DiffCalc calc(int mods, double singletap_threshold, Controller c)
    {
        reset();
        ...
        if (radius < CIRCLESIZE_BUFF_THRESHOLD)
        {
            scaling_factor *= 1.0 +
                ...
                * c.getVariable("high cs buff", 0.02);
        }
        ...
        /* speed and aim stars */
        speed = calc_individual(DIFF_SPEED, c);
        aim = calc_individual(DIFF_AIM, c);
    ...
    */
    public DiffCalc calc(int mods, Controller c) {
        return calc(mods, DEFAULT_SINGLETAP_THRESHOLD, c);
    }
    ...
    * @see DiffCalc#DEFAULT_SINGLETAP_THRESHOLD
    */
    public DiffCalc calc(Controller c) {
        return calc(MODS_NOMOD, DEFAULT_SINGLETAP_THRESHOLD, c);
    }
    ...
    */
    public DiffCalc calc(Map beatmap, int mods,
        double singletap_threshold, Controller c)
    {
        this.beatmap = beatmap;
        return calc(mods, singletap_threshold, c);
    }
    ...
    * @see DiffCalc#DEFAULT_SINGLETAP_THRESHOLD
    */
    public DiffCalc calc(Map beatmap, int mods, Controller c) {
        return calc(beatmap, mods, DEFAULT_SINGLETAP_THRESHOLD, c);
    }
    ...
    * @see DiffCalc#DEFAULT_SINGLETAP_THRESHOLD
    */
    public DiffCalc calc(Map beatmap, Controller c)
    {
        return calc(beatmap, MODS_NOMOD,
            DEFAULT_SINGLETAP_THRESHOLD, c);
    }
}
...
public static class PPv2
{
    ...
    */
    private PPv2(double aim_stars, double speed_stars,
        ...
        int score_version, Map beatmap, Controller c)
    {
        if (beatmap != null)
        {
            mode = beatmap.mode;
        ...
        double length_bonus = 0.95 + c.getVariable("length bonus 1", 0.4) *
            Math.min(1.0, nobjects_over_2k);

        if (nobjects > 2000) {
            length_bonus += Math.log10(nobjects_over_2k) * c.getVariable("length bonus 2", 0.5);
        }
        ...
        if (mapstats.ar > 10.33) {
            ar_bonus += c.getVariable("AR bonus", 0.45) * (mapstats.ar - 10.33);
        }

        else if (mapstats.ar < 8.0)
        {
            double low_ar_bonus = c.getVariable("low AR bonus", 0.01) * (8.0 - mapstats.ar);
        ...
        if ((mods & MODS_HD) != 0) {
            aim *= c.getVariable("HD bonus", 1.18);
        }

        if ((mods & MODS_FL) != 0) {
            aim *= c.getVariable("FL bonus", 1.45) * length_bonus;
        }
        ...
        aim *= acc_bonus;
        aim *= od_bonus;
        aim *= c.getVariable("aim weight", 1);
        ...
        speed *= acc_bonus;
        speed *= od_bonus;
        speed *= c.getVariable("speed weight", 1);
        
        /* acc pp ---------------------------------------------- */
        acc = Math.pow(c.getVariable("OD acc mod", 1.52163), mapstats.od) *
            Math.pow(real_acc, 24.0) * c.getVariable("acc weight", 2.83);
    ...
    /** @see PPv2Parameters */
    public PPv2(PPv2Parameters p, Controller c)
    {
    ...
    * @see Koohii.PPv2#Koohii.PPv2(Koohii.PPv2Parameters)
    */
    public PPv2(double aim_stars, double speed_stars, Map b, Controller c)
    {
